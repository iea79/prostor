...
<div id="user-submitted-posts">
	...
	<form id="usp_form" method="post" enctype="multipart/form-data" action="" class="form formPost">
		<div id="usp-error-message" class="usp-callout-failure usp-hidden form__message"><?php esc_html_e('Please complete the required fields.', 'usp'); ?></div>
		<?php echo usp_error_message();

		if (isset($_GET['success']) && $_GET['success'] == '1') :
			echo '<div id="usp-success-message" class="form__message">'. $usp_options['success-message'] .'</div>';
		else :

        if ($usp_options['usp_title'] == 'show' || $usp_options['usp_title'] == 'optn') { ?>

		<fieldset class="usp-title form__row form__row">
			<label for="user-submitted-title"><?php esc_html_e('Ваше имя', 'usp'); ?></label>
			<input id="user-submitted-title" name="user-submitted-title" type="text" value="" placeholder="Введите имя"<?php if (usp_check_required('usp_title')) echo $usp_required; ?> class="usp-input">
		</fieldset>
		<?php }

		if (($usp_options['usp_name'] == 'show' || $usp_options['usp_name'] == 'optn') && ($usp_display_name)) { ?>

		<fieldset class="usp-name form__row">
		...
		<fieldset class="usp-email form__row form__row">
			<label for="user-submitted-email"><?php esc_html_e('E-mail', 'usp'); ?></label>
		...
		<fieldset class="usp-url form__row">
		...
		<fieldset class="usp-tags form__row">
		...
		<fieldset class="usp-custom form__row">
		...
		<fieldset class="usp-content form__row form__row">
			...
			<label for="user-submitted-content"><?php esc_html_e('Отзыв', 'usp'); ?></label>
		...
		<fieldset class="usp-recaptcha form__row">
		...
		<fieldset class="usp-images form__row">
			<label for="user-submitted-image"></label>
			<div id="usp-upload-message"></div>
			<div id="user-submitted-image" class="formUpload">
			<?php // upload files
			...
			if (empty($usp_addAnother)) $usp_addAnother = '<a id="usp_add-another" class="usp-no-js btn btn_border_primary">'. esc_html__('Add another image', 'usp') .'</a>';

			if ($usp_minImages > 0) : ?>
				<?php for ($i = 0; $i < $usp_minImages; $i++) : ?>
                <label class="form__upload">
                    <input name="user-submitted-image[]" type="file" size="25"<?php echo $usp_required; ?> class="usp-input usp-clone" visible>
                    <span class="btn btn_primary"><?php esc_html_e('Upload an Image', 'usp'); ?></span>
                </label>
				<?php endfor; ?>
			...
			<?php else : ?>
                ...
                <label class="form__upload">
    				<input name="user-submitted-image[]" type="file" size="25" class="usp-input usp-clone" data-parsley-excluded="true" visible>
                    <span class="btn btn_primary"><?php esc_html_e('Upload an Image', 'usp'); ?></span>
                </label>
		...
		<div id="usp-submit" class="form__row usp-submit">
            ...
            <div class="form__privacy">
                <div class="form__check">
                    ...
                    <span>Я согласен на <a href="/privacy-policy">обработку персональных данных</a></span>
                </div>
            </div>

			<button type="submit" class="usp-submit btn btn_primary" id="user-submitted-post" name="user-submitted-post" value=""><?php esc_attr_e('Оставить отзыв', 'usp'); ?></button>
